...
Level- Easy
---------------------------
You are conductor of a bus .There are n chairs and n passengers in the bus. You are given an array chairs of length n, 
where chairs[i] is the position of the ith chair. You are also given the array passengers of length n, where passengers[j] 
is the position of the jth passenger.
...
Return the minimum number of moves required to move each passenger to a chair such that no two passengers are in the same chair and
every passenger gets a chair.
...
class Solution {
    public static int findMoves(int n, int[] chairs, int[] passengers) {
        ...
        //then, traverse over array of chairs and find absolute difference with array of passengers and add into result
